...
function updateQuote() {
  ...
  let color = randColor();
  let tweetLink = createTweetLink(randomQuote.text, randomQuote.source);
  let tumblrLink = createTumblrLink(randomQuote.text, randomQuote.source);

  // document.querySelector("#text").textContent = randomQuote.text;
  // document.querySelector("#quote-box").style = `color: ${color}`;
  // document.querySelector("#author").textContent = randomQuote.source;
  // document.querySelector("button").style.backgroundColor = color;
  // document.querySelector("#tweet-quote").style.backgroundColor = color;
  // document.querySelector("#tweet-quote").href = tweetLink;
  // document.querySelector("#tumblr-quote").style.backgroundColor = color;
  // document.querySelector("#tumblr-quote").href = tumblrLink;
  ...
  $("#author").fadeOut(300, function () {
    $(this).text(randomQuote.source).fadeIn(300);
  });
  $("button").css("background-color", color);
  $("#tweet-quote").css("background-color", color);
  $("#tweet-quote").attr("href", tweetLink);
  $("#tumblr-quote").css("background-color", color);
...
function randColor() {
  ...
  let s = Math.floor(Math.random() * 20) + 40;
  let l = Math.floor(Math.random() * 20) + 40;

  return `hsl(${h}, ${s}%, ${l}%)`;
}

function createTweetLink(quote, author) {
  var baseUrl = "https://twitter.com/intent/tweet";
  var hashtags = "quotes";
  var related = "freecodecamp";
  var text = encodeURIComponent('"' + quote + '" ' + author);

  return baseUrl + "?hashtags=" + hashtags + "&related=" + related + "&text=" + text;
}

function createTumblrLink(quote, author) {
  var baseUrl = "https://www.tumblr.com/widgets/share/tool";
  var tags = "quotes";
  var caption = encodeURIComponent('"' + quote + '" ' + author);
  var posttype = "quote";

  return baseUrl + "?tags=" + tags + "&caption=" + caption + "&posttype=" + posttype;
}
